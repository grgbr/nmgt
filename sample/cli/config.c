...
struct cli_config_cmd {
	struct cli_cmd         super;
	struct cli_lyd_table * table;
};

static int
cli_config_show_table(const struct cli_cmd *     command,
                      const struct cli_context * context)
{
	cli_cmd_assert(command);
	cli_lyd_table_assert(((struct cli_config_cmd *)command)->table);
	cli_assert_context(context);

	struct cli_config_cmd * cmd = (struct cli_config_cmd *)command;
	int                     ret;
...
static int
cli_config_show_json(const struct cli_cmd *     command,
...
static int
cli_config_show_xml(const struct cli_cmd *     command,
                    const struct lysc_node *   schema,
                    const struct cli_context * context)
{
	sr_data_t * data;
...
struct cli_config_work {
	struct cli_dir_work  super;
	enum cli_data_format format;
};

static int
cli_config_exec_work(struct cli_work * work, struct cli_context * context)
{
	const struct cli_config_work * wk = (const struct cli_config_work *)
	                                    work;
	...
	case CLI_TABLE_DATA_FMT:
		ret = cli_config_show_table(wk->super.cmd, context);
		break;

	case CLI_JSON_DATA_FMT:
		ret = cli_config_show_json(wk->super.cmd,
		                           dir->sch_node,
		                           context);
		break;

	case CLI_XML_DATA_FMT:
		ret = cli_config_show_xml(wk->super.cmd,
		                          dir->sch_node,
		                          context);
		break;
...
static const struct cli_work_ops cli_config_work_ops = {
	.exec    = cli_config_exec_work,
	.release = cli_dir_work_release
};

static int
cli_config_parse_cmd(const struct cli_cmd * command,
                     ...
                     const char * const     argv[])
{
	...
	struct cli_config_work * wk;
	int                      ret;

	/* Cannot fail. */
	wk = (struct cli_config_work *)
	     cli_dir_work_create(sizeof(*wk), command, &cli_config_work_ops);
...
static void
cli_config_fini_cmd(struct cli_cmd * command)
{
	struct cli_lyd_table * tbl = ((struct cli_config_cmd *)command)->table;

	cli_lyd_table_fini(tbl);
	cli_free(tbl);
}

static const struct cli_cmd_ops cli_config_cmd_ops = {
	.parse    = cli_config_parse_cmd,
	.complete = cli_cmd_complete_args,
	.fini     = cli_config_fini_cmd
};

static int
cli_config_on_table_match(const struct cli_arg * argument __cli_unused,
                          ...
                          void *                 data)
{
	((struct cli_config_work *)data)->format = CLI_TABLE_DATA_FMT;

	return 0;
}

static int
cli_config_on_json_match(const struct cli_arg * argument __cli_unused,
                         const struct cli_cmd * command __cli_unused,
                         const struct cli_dir * directory __cli_unused,
                         struct cli_context *   context __cli_unused,
                         int                    argc __cli_unused,
                         ...
                         void *                 data)
{
	((struct cli_config_work *)data)->format = CLI_JSON_DATA_FMT;

	return 0;
}

static int
cli_config_on_xml_match(const struct cli_arg * argument __cli_unused,
                        const struct cli_cmd * command __cli_unused,
                        const struct cli_dir * directory __cli_unused,
                        struct cli_context *   context __cli_unused,
                        int                    argc __cli_unused,
                        ...
                        void *                 data)
{
	((struct cli_config_work *)data)->format = CLI_XML_DATA_FMT;

	return 0;
}

static const struct cli_arg_kword_term cli_config_format_terms[] = {
	CLI_ARG_KWORD_TERM("table", cli_config_on_table_match),
	CLI_ARG_KWORD_TERM("json",  cli_config_on_json_match),
	CLI_ARG_KWORD_TERM("xml",   cli_config_on_xml_match),
};

static bool
cli_config_filter_node(const struct lysc_node * node)
{
	cli_assert(node);
...
static struct cli_config_cmd *
cli_config_create_cmd(struct cli_dir *           directory,
                      const struct cli_context * context)
{
	cli_dir_assert(directory);
	 ...
	 * with the "config" name argument.
	 */
	cli_assert(sizeof("config") <= CLI_ARG_MAX);
	...
	/*
	 * Cannot fail since calling cli_config_make_cmd() previously made sure
	...
	cli_arg_createn_add_kword_parm(&fmt,
	                               "format",
	                               cli_config_format_terms,
	                               cli_array_nr(cli_config_format_terms),
                    ...
                    const struct cli_context *    context)
{
	...
	/*
	 * Make sure that there is at least one leaf suitable for the config
	 * command to display available informations.
	 */
	if (cli_config_filter_node(&leaf->node)) {
		struct cli_cmd * cmd;

		cmd = cli_dir_find_cmd(directory, "config");
		if (cmd)
			return cmd;

		return (struct cli_cmd *)cli_config_create_cmd(directory,
		                                               context);
	}
	else
		return NULL;
